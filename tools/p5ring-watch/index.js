#!/usr/bin/env node
// =============================================================================
// p5ring-watch — watches a local folder, pushes .js sketches to a p5-ring over
...
//
// The folder's `current.js` is the one p5-ring runs. Any other .js file is
...
async function findPort() {
    ...
    // p5-ring USB product is "p5-ring"; manufacturer is "Processing Foundation Friends"
    const match = list.find(p =>
        (p.productId && /p5-?ring|q-?ring/i.test(p.productName || '')) ||
        /p5-ring/i.test(p.manufacturer || '') ||
        /usbmodem/i.test(p.path || '') ||
        /COM\d+/.test(p.path || '')
    );
    if (!match) {
        console.error('No p5-ring device found. Pass --port explicitly.');
...
async function main() {
    ...
    console.log(`[p5ring-watch] using port: ${found}`);
    console.log(`[p5ring-watch] watching:   ${watchDir}`);
    ...
    function upload(file) {
        queue = queue.then(async () => {
            ...
            console.log(`[p5ring-watch] uploading ${rel} (${body.length} bytes) -> ${devicePath}`);
    ...
    console.log('[p5ring-watch] ready. Save a .js file to push.');
}
